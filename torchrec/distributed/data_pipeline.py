...
def pipeline_cuda_copying(dataloader_iter: Iterator[In], device: torch.device) -> Iterator[Out]:
    return CudaCopyingPipeline(dataloader_iter, device)


def pipeline_sparse_dist(dataloader_iter: Iterator[In], module: torch.nn.Module, device: torch.device) -> Tuple[Iterator[Out], torch.nn.Module]:
    pipeline = SparseDistPipeline(dataloader_iter, module, device)
...
class CudaCopyingPipeline(Iterator[Out], Generic[In, Out]):
    """
    ...
    def __next__(self) -> Out:
        ...
        with record_function("## copy_batch_to_gpu ##"):
            with torch.cuda.stream(self._memcpy_stream):
                self._cur_batch = _to_device(next_batch, self._device, non_blocking=True)
...
class SparseDistPipeline(Iterator[Out], Generic[In, Out]):
    """
        ...
        with record_function("## sparse_data_dist ##"):
            with torch.cuda.stream(self._data_dist_stream):
                ...
                if self._event is not None:
                    ...
                    self._data_dist_stream.wait_event(self._event)
